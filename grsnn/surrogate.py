...
class normalize(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            x = ctx.saved_tensors[0]
            mask = (x < 0)
            grad_x = grad_output.masked_fill_(mask, 0)
            # alpha is the threshold
            grad_x = grad_x / (x + ctx.alpha).masked_fill_(mask, 1.)
        return grad_x, None

class Normalize(SurrogateFunctionBase):
    def __init__(self, alpha=2.0, spiking=True):
        super().__init__(alpha, spiking)
    ...
    @staticmethod
    def spiking_function(x, alpha):
        return normalize.apply(x, alpha)
