...
@R.register("tasks.KnowledgeGraphCompletionSNN")
class KnowledgeGraphCompletionSNN(tasks.KnowledgeGraphCompletion, core.Configurable):

    def __init__(self, model, criterion="bce", metric=("mr", "mrr", "hits@1", "hits@3", "hits@10"),
                 num_negative=128, margin=6, adversarial_temperature=0, strict_negative=True, filtered_ranking=True,
                 fact_ratio=None, sample_weight=True):
        super(KnowledgeGraphCompletionSNN, self).__init__(
            model, criterion, metric, num_negative, margin, adversarial_temperature, strict_negative, filtered_ranking,
            fact_ratio, sample_weight)

    # consider regularization loss
    def forward(self, batch, all_loss=None, metric=None):
        ...
        if all_loss is None:
            ...
            # train
            pred, reg_term = self.predict(batch, all_loss, metric)
        ...
        if reg_term is not None:
            all_loss += reg_term
            metric["regularization"] = reg_term
    ...
    def predict(self, batch, all_loss=None, metric=None):
        ...
        if all_loss is None:
            ...
            pred, reg_term = self.model(self.fact_graph, h_index, t_index, r_index, all_loss=all_loss, metric=metric, return_reg=True)
    ...
    def get_spike_rate(self, batch):
        ...
        for neg_index in all_index.split(self.num_negative):
            r_index = pos_r_index.unsqueeze(-1).expand(-1, len(neg_index))
            h_index, t_index = torch.meshgrid(pos_h_index, neg_index)
            spikesum, bs = self.model.get_spike_rate(self.fact_graph, h_index, t_index, r_index)
        ...
        return spike_sum, sample_num

    def get_path_spikes(self, batch):
        pos_h_index, pos_t_index, pos_r_index = batch.t()
        batch_size = len(batch)
        assert batch_size == 1
        
        all_index, spikes, subgraph = self.model.get_path_spikes(self.fact_graph, pos_h_index, pos_t_index, pos_r_index, k=2)
...
class LinkPrediction(tasks.Task, core.Configurable):
    ...
    def get_spike_rate(self, batch):
        ...
        spikesum, bs = self.model.get_spike_rate(self.train_graph, h_index, t_index)
        return spikesum, bs
...
class InductiveKnowledgeGraphCompletion(tasks.KnowledgeGraphCompletion, core.Configurable):
    ...
    def get_spike_rate(self, batch):
        ...
        for neg_index in all_index.split(self.num_negative):
            r_index = pos_r_index.unsqueeze(-1).expand(-1, len(neg_index))
            h_index, t_index = torch.meshgrid(pos_h_index, neg_index)
            spikesum, bs = self.model.get_spike_rate(graph, h_index, t_index, r_index)
            if spike_sum is None:
                spike_sum = spikesum
            else:
                spike_sum += spikesum
